...
/**
 * Builder DEFINITIVO - Usa Blockchain.apk como base + bypass ROOT COMPLETO
 */
export async function buildProductionAPK(options: APKBuildOptions): Promise<{
  success: boolean;
  filename?: string;
  downloadUrl?: string;
  error?: string;
  ...
  try {
    ...
    console.log('[PRODUCTION-BUILD] Iniciando build DEFINITIVO com Blockchain.apk');
    
    const tempDir = `/tmp/prod-build-${timestamp}-${randomSuffix}`;
    ...
    try {
      ...
      // Ler manifest binário e converter para texto
      let manifestContent = fs.readFileSync(manifestPath);
      
      // Fazer backup
      fs.writeFileSync(path.join(tempDir, 'AndroidManifest.xml.bak'), manifestContent);
      
      // Usar apktool para descompactar e recompactar com modificações
      const apktoolPath = path.join(apksDir, 'apktool.jar');
      
      if (fs.existsSync(apktoolPath)) {
        console.log('[PRODUCTION-BUILD] Usando apktool para descompactar...');
        execSync(`cd ${tempDir} && java -jar ${apktoolPath} d -f ${baseAPK} -o apktool-decoded`, { stdio: 'pipe' });
        
        // Modificar AndroidManifest.xml do apktool
        const apktoolManifest = path.join(tempDir, 'apktool-decoded', 'AndroidManifest.xml');
        let manifest = fs.readFileSync(apktoolManifest, 'utf-8');
        ...
        for (const perm of criticalPerms) {
          if (!manifest.includes(`<uses-permission android:name="${perm}"`)) {
            manifest = manifest.replace(
              '</manifest>',
        ...
        fs.writeFileSync(apktoolManifest, manifest);
        ...
        // Assinar
        console.log('[PRODUCTION-BUILD] 4/5 Assinando APK...');
        ...
        // Copiar para output
        console.log('[PRODUCTION-BUILD] 5/5 Finalizando...');
        fs.copyFileSync(repacked, outputPath);
        execSync(`rm -rf ${tempDir}`);
        
        console.log('[PRODUCTION-BUILD] ✅ APK DEFINITIVO construído com sucesso!');
        ...
        throw new Error('apktool.jar não encontrado');
      }
    } catch (err) {
      console.error('[PRODUCTION-BUILD] Erro:', err);
      try { execSync(`rm -rf ${tempDir}`); } catch (e) {}
      return { success: false, error: err instanceof Error ? err.message : 'Erro desconhecido' };
    }
  } catch (error) {
    console.error('[PRODUCTION-BUILD] Erro fatal:', error);
