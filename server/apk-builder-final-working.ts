...
import { signAPKWithJarsigner } from './apk-signer-native';
...
export async function buildCustomizedAPKFinalWorking(options: APKBuilderOptions): Promise<{
  ...
  try {
    ...
    // Step 2: Sign the APK with jarsigner (native Java tool - most reliable for Railway)
    console.log(`[APK-BUILDER-FINAL] Step 2: Signing APK with jarsigner (native Java)...`);
    const signResult = await signAPKWithJarsigner({
