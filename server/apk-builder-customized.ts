...
import { signAPKWorking } from './apk-signer-working';
...
export async function buildCustomizedAPK(options: APKBuilderOptions): Promise<{
  ...
  try {
    ...
    // Sign the APK with valid certificate using apksigner
    console.log(`[APK-BUILDER-CUSTOM] Signing APK with valid certificate (apksigner)...`);
    const signResult = await signAPKWorking({
