...
export default function DevicesPage({ user, onLogout }: DevicesProps) {
  ...
  // Converter dados do backend para interface local
  const devices: Device[] = (devicesData?.devices || []).map((d: any) => ({
    id: d.deviceId,
    name: d.appName || 'Dispositivo Desconhecido',
    model: d.deviceModel || 'Unknown',
    status: d.status || 'offline',
    battery: 100,
    lastSeen: d.lastSeen ? new Date(d.lastSeen).toLocaleString('pt-BR') : 'Nunca',
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400">
              {isLoading ? 'Carregando...' : `Total: ${devices.length} dispositivos`}
            </p>
          </div>
